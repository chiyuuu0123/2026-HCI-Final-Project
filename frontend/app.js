...
const QUIZ_DOCUMENT_TEXT_LIMIT = 52000;
const QUIZ_DEFAULT_COUNTS = {
  choice: 4,
  judge: 3,
  fill: 3,
  essay: 2,
};
const QUIZ_TRUSTED_SOURCES = new Set(["qwen-document-quiz"]);
...
const GRAPH_GENERATION_STEPS = {
  idle: { title: "等待生成", detail: "导入资料后可用 Qwen / 本地规则生成课程图谱。测验需在自动测验页独立生成。", progress: 0 },
  extracting: { title: "正在整理资料", detail: "提取 PDF / Markdown 文本，准备生成课程知识结构。", progress: 18 },
  ai: { title: LONGLONG_THINKING_LINE, detail: "龙龙正在生成结构化知识图谱 JSON。", progress: 56 },
...
function extractDocumentQuizKeywords(text = "", limit = 18) {
  const tokens = String(text || "")
    .match(/[\u4e00-\u9fa5]{2,10}|[A-Za-z][A-Za-z0-9_-]{2,}/g) || [];
  const stopWords = new Set(["可以", "需要", "因此", "因为", "通过", "进行", "以及", "其中", "如果", "对于", "使用", "系统", "用户", "课程", "资料", "the", "and", "for", "with", "this", "that"]);
  const scores = new Map();
  tokens.forEach((token) => {
    const normalized = token.trim();
    if (!normalized || stopWords.has(normalized.toLowerCase())) return;
    if (/^\d+$/.test(normalized)) return;
    scores.set(normalized, (scores.get(normalized) || 0) + (normalized.length >= 4 ? 2 : 1));
  });
...
function getQuizDocumentQuality(documents = []) {
  const text = documents.map((documentMeta) => documentMeta.text || "").join("\n\n");
  const meaningfulLines = String(text || "")
    .split(/\n+/)
    .map(normalizeStudyTextLine)
    .filter((line) => line.length >= 12 && !isBoilerplateStudyLine(line));
  const keywords = extractDocumentQuizKeywords(text, 40);
  const meaningfulChars = meaningfulLines.join("\n").length;
  const ok = meaningfulChars >= QUIZ_MIN_MEANINGFUL_CHARS
    && meaningfulLines.length >= QUIZ_MIN_MEANINGFUL_LINES
    && keywords.length >= QUIZ_MIN_KEYWORDS;
  return {
    ok,
    meaningfulChars,
    ...
    message: ok
      ? "资料质量足够生成测验。"
      : `资料可用正文不足，暂不生成测验题库。需要至少 ${QUIZ_MIN_MEANINGFUL_CHARS} 字有效正文、${QUIZ_MIN_MEANINGFUL_LINES} 行有效内容、${QUIZ_MIN_KEYWORDS} 个可识别关键词；当前约 ${meaningfulChars} 字、${meaningfulLines.length} 行、${keywords.length} 个关键词。`,
  };
}
...
function normalizeStandaloneQuizType(type) {
  const rawType = String(type || "choice").trim().toLowerCase();
  if (["single_choice", "single-choice", "choice", "select", "radio"].includes(rawType) || /单选|选择/.test(rawType)) return "choice";
  if (["judge", "true_false", "true-false", "boolean"].includes(rawType) || /判断|正误/.test(rawType)) return "judge";
  if (["fill_blank", "fill-blank", "fill", "blank"].includes(rawType) || /填空/.test(rawType)) return "fill";
  if (["essay", "short", "short_answer", "short-answer"].includes(rawType) || /简答|问答|论述/.test(rawType)) return "essay";
  return normalizeQuestionType(rawType);
}

function normalizeQuizDifficulty(value) {
  const raw = String(value || "中等").trim();
  if (/基础|简单|easy/i.test(raw)) return "基础";
  if (/较难|困难|hard/i.test(raw)) return "较难";
  return "中等";
}

function normalizeQuizCounts(counts = {}) {
  return {
    choice: Math.max(0, Math.min(12, Number(counts.choice ?? counts.single_choice ?? QUIZ_DEFAULT_COUNTS.choice) || 0)),
    judge: Math.max(0, Math.min(12, Number(counts.judge ?? QUIZ_DEFAULT_COUNTS.judge) || 0)),
    fill: Math.max(0, Math.min(12, Number(counts.fill ?? counts.fill_blank ?? QUIZ_DEFAULT_COUNTS.fill) || 0)),
    essay: Math.max(0, Math.min(8, Number(counts.essay ?? QUIZ_DEFAULT_COUNTS.essay) || 0)),
  };
}

function getQuizRequestedTotal(counts = {}) {
  const normalized = normalizeQuizCounts(counts);
  return normalized.choice + normalized.judge + normalized.fill + normalized.essay;
}

function getQuizTypeCounts(questions = []) {
  return questions.reduce((result, question) => {
    const type = question.type === "short" ? "essay" : question.type;
    if (Object.prototype.hasOwnProperty.call(result, type)) result[type] += 1;
    return result;
  }, { choice: 0, judge: 0, fill: 0, essay: 0 });
}

function validateGeneratedQuizQuestions(questions = [], counts = {}) {
  const normalizedCounts = normalizeQuizCounts(counts);
  const requestedTotal = getQuizRequestedTotal(normalizedCounts);
  if (requestedTotal <= 0) {
    return { ok: false, message: "请至少设置 1 道题。" };
  }
  const typeCounts = getQuizTypeCounts(questions);
  const missing = Object.entries(normalizedCounts)
    .filter(([type, count]) => count > 0 && typeCounts[type] < count)
    .map(([type, count]) => `${getQuizTypeLabel(type)}需要 ${count} 道，实际 ${typeCounts[type]} 道`);
  if (questions.length < requestedTotal || missing.length) {
    return {
      ok: false,
      message: `AI 返回题量不足或题型不完整，暂不生成题库。${missing.join("；") || `需要 ${requestedTotal} 道，实际 ${questions.length} 道`}`,
    };
  }
  return { ok: true, message: "题库生成结果合格。" };
}

function getQuizQuestionTopic(question = {}) {
  return String(question.topic || question.knowledgePoint || question.source || "资料综合").trim() || "资料综合";
}

function normalizeStandaloneQuizQuestion(question, fallback = {}) {
  if (!question) return null;
  const type = normalizeStandaloneQuizType(question.type || fallback.type);
  const prompt = String(question.prompt || question.question || "").trim();
  if (!prompt || hasQuestionStemFieldLeakage(prompt)) return null;

  const source = String(question.source || fallback.source || "课程资料").trim();
  const topic = String(question.topic || question.knowledgePoint || question.nodeId || fallback.topic || source || "资料综合").trim();
  const normalized = {
    id: question.id || createId("quiz"),
    topic,
    type,
    difficulty: normalizeQuizDifficulty(question.difficulty || fallback.difficulty),
    prompt,
    options: normalizeQuestionOptions(question.options),
    answer: question.answer,
    keywords: Array.isArray(question.keywords) ? question.keywords.map(String).slice(0, 8) : [],
    pairs: Array.isArray(question.pairs) ? question.pairs : [],
    sampleAnswer: String(question.sampleAnswer || question.referenceAnswer || question.answerText || "").trim(),
    explanation: String(question.analysis || question.explanation || question.reason || "").trim(),
    source,
    sourceSnippet: String(question.sourceSnippet || question.snippet || fallback.sourceSnippet || "").trim(),
  };
  if (hasQuestionOptionFieldLeakage(normalized.options)) return null;
  ...
  if (normalized.type === "choice") {
    normalized.options = normalized.options.filter(Boolean).slice(0, 4);
    if (normalized.options.length < 2) return null;
    normalized.answer = coerceChoiceAnswerIndex(normalized.answer, normalized.options);
    if (normalized.answer < 0) return null;
  }

  if (normalized.type === "judge" && typeof normalized.answer !== "boolean") {
    normalized.answer = /^(true|正确|对|是|yes)$/i.test(String(normalized.answer).trim());
  }

  if (normalized.type === "fill") {
    normalized.answer = String(normalized.answer || normalized.sampleAnswer || "").trim();
    if (!normalized.answer) return null;
    if (!normalized.prompt.includes("__")) normalized.prompt = `${normalized.prompt}（填空）`;
    normalized.sampleAnswer = normalized.answer;
  }

  if (normalized.type === "essay") {
    normalized.sampleAnswer = String(normalized.sampleAnswer || normalized.answer || "").trim();
    normalized.answer = normalized.sampleAnswer;
    if (!normalized.sampleAnswer && !normalized.explanation) return null;
  }
  ...
  if (!normalized.explanation) {
    normalized.explanation = normalized.sourceSnippet
      ? "解析来自上传资料中的对应片段。"
      : "请对照参考答案复习相关资料内容。";
  }
...
function normalizeQuizPayloadQuestions(payload, documents = []) {
  const sourceTitle = documents[0]?.title || "课程资料";
  const buckets = [
    ...(Array.isArray(payload?.single_choice) ? payload.single_choice.map((item) => ({ ...item, type: "single_choice" })) : []),
    ...(Array.isArray(payload?.judge) ? payload.judge.map((item) => ({ ...item, type: "judge" })) : []),
    ...(Array.isArray(payload?.fill_blank) ? payload.fill_blank.map((item) => ({ ...item, type: "fill_blank" })) : []),
    ...(Array.isArray(payload?.essay) ? payload.essay.map((item) => ({ ...item, type: "essay" })) : []),
    ...(Array.isArray(payload?.quiz?.questions) ? payload.quiz.questions : []),
    ...(Array.isArray(payload?.questions) ? payload.questions : []),
  ];
  return uniqueQuestions(buckets
    .map((question) => normalizeStandaloneQuizQuestion(question, { source: sourceTitle }))
    .filter(Boolean));
}
...
function getQuestionPriority(question, context = {}) {
  ...
  const topicAttempts = studyModule.attempts.filter((attempt) => attempt.topic === question.topic);
  const topicAccuracy = topicAttempts.length
    ? Math.round((topicAttempts.filter((attempt) => attempt.correct).length / topicAttempts.length) * 100)
    : null;
  ...
  return [
    dueMistake ? 70 : 0,
    topicAccuracy === null ? 12 : topicAccuracy < 60 ? 34 : topicAccuracy < 78 ? 18 : 4,
    lastAttempt && !lastAttempt.correct ? 26 : 0,
    history.length === 0 ? 18 : Math.max(0, 10 - history.length * 2),
    difficultyBoost,
    (question.type === "short" || question.type === "essay") && mode !== "diagnostic" ? 6 : 0,
...
function getTopicQuestions(topicId, difficulty = "all") {
  const studyModule = getStudyModule();
  const questions = studyModule.quiz.questions || [];
  ...
  if (mode === "weak") {
    const weakTopics = new Set(studyModule.mistakes.filter((mistake) => !mistake.reviewed).map((mistake) => mistake.topic));
  ...
  const filtered = questions
    .filter((question) => getQuizQuestionTopic(question) === topicId || question.source === topicId)
...
function renderQuizScopeControls() {
  ...
  const counts = normalizeQuizCounts(studyModule.quiz.counts);
  if (modeSelect) modeSelect.value = studyModule.quiz.mode || "adaptive";
  if (scopeSelect) {
    const topics = Array.from(new Set((studyModule.quiz.questions || []).map(getQuizQuestionTopic).filter(Boolean))).slice(0, 40);
    scopeSelect.innerHTML = `<option value="all">整份资料</option>${topics.map((topic) => `<option value="${escapeHtml(topic)}">${escapeHtml(topic)}</option>`).join("")}`;
  ...
  document.querySelectorAll("[data-quiz-count]").forEach((input) => {
    const key = input.dataset.quizCount;
...
function renderQuizModule() {
  ...
  if (!question) {
    ...
    if (questionNode) questionNode.textContent = "还没有可用题库，请导入质量足够的资料后点击“生成题组”。";
    if (answerList) answerList.innerHTML = "";
    if (feedback) {
      feedback.textContent = "资料正文不足、AI 请求失败或模型没有返回合格 JSON 时，系统不会生成本地兜底题。";
  ...
  if (answerList) {
    const draftAnswer = studyModule.quiz.drafts?.[question.id] || "";
    if (question.type === "choice") {
      ...
      answerList.innerHTML = `
        <input class="quiz-fill-answer" id="quiz-fill-answer" type="text" placeholder="输入填空答案" value="${escapeHtml(draftAnswer)}" />
        <button class="primary-action" data-quiz-action="submit-fill">提交填空</button>
      `;
    } else if (question.type === "short" || question.type === "essay") {
      answerList.innerHTML = `
        <textarea class="quiz-short-answer" id="quiz-short-answer" rows="5" placeholder="写下你的简答内容">${escapeHtml(draftAnswer)}</textarea>
        <button class="primary-action" data-quiz-action="submit-essay">提交简答</button>
...
function renderQuizSidebar(questions = getCurrentQuizQuestions()) {
  ...
  if (scoreLabel) {
    const sourceLabel = studyModule.quiz.source === "qwen-document-quiz"
      ? "Qwen 文档题库"
      : "题库未生成";
  ...
  if (topicList) {
    const topics = Array.from(new Set((studyModule.quiz.questions || []).map(getQuizQuestionTopic).filter(Boolean))).slice(0, 16);
    topicList.innerHTML = topics.length
      ? topics
      .map((topic) => {
        const topicAttempts = studyModule.attempts.filter((attempt) => attempt.topic === topic);
        const correct = topicAttempts.filter((attempt) => attempt.correct).length;
        const accuracy = topicAttempts.length ? Math.round((correct / topicAttempts.length) * 100) : null;
        const hasMistake = studyModule.mistakes.some((mistake) => mistake.topic === topic && !mistake.reviewed);
        const className = [
          topic === scope ? "current" : "",
          accuracy !== null && accuracy >= 80 ? "done" : "",
          hasMistake ? "weak-topic" : "",
        ].filter(Boolean).join(" ");
        return `<button class="${className}" data-report-review-topic="${escapeHtml(topic)}">${escapeHtml(topic)}${accuracy === null ? "" : ` ${accuracy}%`}</button>`;
      })
      .join("")
      : `<span>生成题库后会显示资料主题</span>`;
  }
      ...
      ${dueMistakes.length
        ? dueMistakes.map((mistake) => `
          <button class="quiz-review-item" data-report-review-topic="${escapeHtml(mistake.topic)}">
            <strong>${escapeHtml(mistake.topic || "资料综合")}</strong>
...
function renderQuizSessionReport(questions = getCurrentQuizQuestions()) {
  ...
  const weakTopics = Array.from(new Set(studyModule.mistakes
    .filter((mistake) => !mistake.reviewed)
    .map((mistake) => mistake.topic || "资料综合")))
    .slice(0, 3);
  const dueCount = getDueReviewMistakes().length;
  const typeSummary = ["choice", "judge", "fill", "essay"]
    .map((type) => `${getQuizTypeLabel(type)} ${questions.filter((question) => question.type === type || (type === "essay" && question.type === "short")).length}`)
    .join(" · ");
  ...
  reportNode.innerHTML = `
    ...
    <div>
      <strong>${weakTopics[0] || "暂无"}</strong>
      <span>优先补强</span>
    </div>
    <div class="quiz-report-actions">
      ${weakTopics.length
        ? weakTopics.map((topic) => `<button class="mini-button" data-report-review-topic="${escapeHtml(topic)}">${escapeHtml(topic)}</button>`).join("")
        : `<span>${escapeHtml(typeSummary || "暂无题型统计")}</span>`}
    </div>
  `;
}
...
function getQuestionCorrectAnswer(question) {
  ...
  if (question.type === "fill") return question.answer || question.sampleAnswer || "";
  if (question.type === "essay") return question.sampleAnswer || question.answer || question.explanation || "";
...
function inferQuizMistakeReason(question, selectedAnswer = "") {
  if (question.type === "short" || question.type === "essay") return "要点缺失";
  if (question.type === "fill") return "关键词记忆偏差";
...
function recordQuizAnswer(selectedAnswer, correct, meta = {}) {
  ...
  if (studyModule.quiz?.drafts) delete studyModule.quiz.drafts[question.id];
...
function cleanFillBlankAnswer(value = "") {
  return String(value || "")
    .trim()
    .replace(/\s+/g, "")
    .replace(/[，。！？、；;,.!?]/g, "")
...
function submitFillQuizAnswer() {
  const question = getCurrentQuizQuestion();
  if (!question) return;
  const answer = document.querySelector("#quiz-fill-answer")?.value || "";
  const expectedValues = String(getQuestionCorrectAnswer(question) || "")
    .split(/[、,，;；/]/)
    .map(cleanFillBlankAnswer)
    .filter(Boolean);
  const selected = cleanFillBlankAnswer(answer);
  const correct = Boolean(selected) && expectedValues.some((expected) => selected === expected);
  recordQuizAnswer(answer.trim() || "未填写", correct);
  showQuizFeedback(correct, question.explanation, getQuestionCorrectAnswer(question));
  refreshAfterQuizAnswer();
}
...
function buildShortAnswerGradePrompt(question, answer) {
  return [
    ...
    "本次评分只依据题目、标准答案、用户作答和来源片段；不要引用知识图谱、graph、node、edge 或 Neo4j。",
    "评分要宽容但有依据，重点看学生是否表达了核心含义，不要求逐字一致。",
    "JSON 格式：",
    '{"score":0,"reason":"评分说明","correct":false,"feedback":"一句具体反馈","matchedPoints":["命中的要点"],"missingPoints":["缺失的要点"]}',
    `题目：${question.prompt}`,
    `学生答案：${answer || "未填写"}`,
...
async function submitShortQuizAnswer() {
  ...
  const submitButton = document.querySelector("[data-quiz-action='submit-essay']");
  if (submitButton) {
...
function generateQuizForTopic(topicId, difficulty = "all") {
  const studyModule = getStudyModule();
  const availableTopics = new Set((studyModule.quiz.questions || []).map(getQuizQuestionTopic));
  studyModule.quiz.scope = topicId === "all" ? "all" : (availableTopics.has(topicId) ? topicId : "all");
...
function buildQuizGenerationPrompt(context = {}, documents = []) {
  const outline = extractDocumentOutlineForAi(documents, 60);
  const counts = normalizeQuizCounts(context.counts);
  return [
    "请只根据上传课程资料生成自动测验题库，只返回严格 JSON，不要 Markdown、注释或解释。",
    "重要约束：本任务与知识图谱完全无关。不要引用、读取或假设任何 graph、node、edge、Neo4j、知识图谱节点或图谱关系；所有题目只能来自下方资料正文/大纲。",
    "标准 JSON 结构必须为：",
    '{"single_choice":[{"question":"题干","options":["A选项","B选项","C选项","D选项"],"answer":"A","analysis":"答案解析","topic":"短主题","source":"资料名","sourceSnippet":"原文证据片段"}],"judge":[{"question":"题干","answer":true,"analysis":"答案解析","topic":"短主题","source":"资料名","sourceSnippet":"原文证据片段"}],"fill_blank":[{"question":"题干（空缺位使用__标识）","answer":"标准答案","analysis":"答案解析","topic":"短主题","source":"资料名","sourceSnippet":"原文证据片段"}],"essay":[{"question":"简答题题干","answer":"标准参考答案","analysis":"评分要点与解析","topic":"短主题","source":"资料名","sourceSnippet":"原文证据片段"}]}',
    "字段纪律：题干只能写题干；单选 options 只能写选项正文，可以不带 A/B/C/D 编号；answer 对单选只写 A/B/C/D，判断只写 true/false；填空题空缺必须用 __。",
    "出题要求：覆盖定义理解、事实判断、关键词填空、应用解释；不要生成脱离资料的通用模板题；每题都要有 sourceSnippet。",
    `练习模式：${context.mode || "adaptive"}；范围：${context.scope || "all"}；难度：${context.difficulty || "all"}`,
    `数量要求：单选 ${counts.choice} 道，判断 ${counts.judge} 道，填空 ${counts.fill} 道，简答 ${counts.essay} 道。`,
    `资料大纲候选：${JSON.stringify(outline)}`,
  ].join("\n");
}

async function regenerateQuizQuestionsFromCurrentGraph(source = "qwen-document-quiz", context = {}) {
  const studyModule = getStudyModule();
  let documents = await buildStudyAiDocumentsForCourse({
    requireCompletePdfOcr: true,
    textLimit: QUIZ_DOCUMENT_TEXT_LIMIT,
  }).catch(() => getCourseStudyDocumentsSnapshot());
  documents = cleanDocumentsForStudyGeneration(documents)
    .map((documentMeta) => ({
      ...documentMeta,
      text: sampleTextWithinLimit(documentMeta.text || "", QUIZ_DOCUMENT_TEXT_LIMIT),
    }))
    .filter((documentMeta) => documentMeta.text);
  const counts = normalizeQuizCounts(context.counts || studyModule.quiz.counts);
  if (!documents.length) {
    throw new Error("没有可用于生成测验的资料正文，请先上传可复制文本的 PDF 或 Markdown。");
  }
  const quality = getQuizDocumentQuality(documents);
  if (!quality.ok) {
    throw new Error(quality.message);
  }
  if (!window.mindStudy?.ai?.askQuestion) {
    throw new Error("当前运行环境没有可用的 Qwen 调用入口，已停止生成测验。");
  }

  await ensureAiConfigured();
  const response = await withTimeout(window.mindStudy.ai.askQuestion({
    question: buildQuizGenerationPrompt({ ...context, counts }, documents),
    documents,
    options: {
      maxChunks: 10,
      maxContextChars: QUIZ_DOCUMENT_TEXT_LIMIT,
      maxTokens: 8192,
      temperature: 0.12,
      persona: false,
      multimodal: true,
  ...
  const parsed = extractJsonFromAiText(response.answer);
  if (!parsed) {
    throw new Error("AI 没有返回可解析的标准 JSON，已停止生成测验题库。");
  }
  const questions = normalizeQuizPayloadQuestions(parsed, documents);
  const validation = validateGeneratedQuizQuestions(questions, counts);
  if (!validation.ok) {
    throw new Error(validation.message);
  }

  studyModule.quiz.questions = questions;
  studyModule.quiz.counts = counts;
  studyModule.quiz.generatedAt = Date.now();
  studyModule.quiz.source = source;
  studyModule.quiz.cursor = 0;
  return true;
}
...
function showQuizGenerationFailure(error) {
  const feedback = document.querySelector("#answer-feedback");
  if (feedback) {
    feedback.className = "answer-feedback wrong";
    feedback.textContent = getAiErrorMessage(error);
  }
}
...
async function handleQuizActionButton(quizActionButton) {
  ...
  if (action === "generate-filtered") {
    ...
    const studyModule = getStudyModule();
    const counts = normalizeQuizCounts(Object.fromEntries(Array.from(document.querySelectorAll("[data-quiz-count]"))
      .map((input) => [input.dataset.quizCount, input.value])));
    if (!getActiveCourse()?.documents?.length && !getCourseStudyDocumentsSnapshot().length) {
      const feedback = document.querySelector("#answer-feedback");
      if (feedback) {
        feedback.className = "answer-feedback wrong";
        feedback.textContent = "请先导入 PDF 或 Markdown 资料，再生成自动测验题库。";
      }
      return;
    }
    ...
    try {
      ...
      studyModule.quiz.counts = counts;
      await regenerateQuizQuestionsFromCurrentGraph("qwen-document-quiz", { mode, scope, difficulty, counts });
      generateQuizForTopic(scope || "all", difficulty);
    } catch (error) {
      showQuizGenerationFailure(error);
    } finally {
  ...
  if (action === "reset") {
    const studyModule = getStudyModule();
    try {
      await regenerateQuizQuestionsFromCurrentGraph("qwen-document-quiz", {
        mode: studyModule.quiz.mode,
        scope: studyModule.quiz.scope,
        difficulty: studyModule.quiz.difficulty,
        counts: studyModule.quiz.counts,
      });
      generateQuizForTopic(studyModule.quiz.scope || "all", studyModule.quiz.difficulty || "all");
    } catch (error) {
      showQuizGenerationFailure(error);
    }
  }
  ...
  if (action === "submit-fill") submitFillQuizAnswer();
  if (action === "submit-short" || action === "submit-essay") submitShortQuizAnswer();
...
function bindQuizActionButtons(root = document) {
  root.querySelectorAll("[data-quiz-action]").forEach((button) => {
    ...
    button.addEventListener("click", (event) => {
      ...
      handleQuizActionButton(button).catch(showQuizGenerationFailure);
    });
  });
}
...
function getReportStats() {
  ...
  const quizTopics = Array.from(new Set([
    ...(studyModule.quiz.questions || []).map(getQuizQuestionTopic),
    ...studyModule.mistakes.map((mistake) => mistake.topic),
  ].filter(Boolean)));
  const weakTopics = quizTopics.length
    ? quizTopics.map((topic) => {
        ...
        const topicWrong = topicAttempts.length - topicCorrect;
        ...
        return {
          ...
          stats: {
            ...
            wrong: topicWrong,
            accuracy: topicAttempts.length ? topicAccuracy : null,
            mastery: clampPercent(topicAttempts.length ? topicAccuracy - topicWrong * 8 : 55),
          },
        };
    ...
    : studyModule.graph.nodes
        ...
        .sort((a, b) => a.stats.mastery - b.stats.mastery);
  const completedChapters = quizTopics.length
    ? new Set(attempts.map((attempt) => attempt.topic).filter(Boolean)).size
    : new Set(studyModule.graph.nodes.filter((node) => getTopicStats(node.id).total > 0).map((node) => node.chapter)).size;
  const allChapters = quizTopics.length
    ? Math.max(1, quizTopics.length)
    : new Set(studyModule.graph.nodes.map((node) => node.chapter)).size || 1;
...
function updateStudyRecommendations() {
  ...
  const weakTopics = stats.weakTopics.slice(0, 4);
  studyModule.recommendations = weakTopics.map((topic) => ({
    id: createId("rec"),
    topic: topic.id,
    title: `复习 ${topic.label}`,
    detail: topic.stats.total
      ? `当前正确率 ${topic.stats.accuracy ?? 0}%，建议重做错题并补看原文出处。`
      : `尚未练习，建议先完成 3 道相关题。`,
    action: "quiz",
  }));
}
...
document.addEventListener("change", (event) => {
  ...
  if (event.target.matches("#quiz-mode-select, #quiz-scope-select, #quiz-difficulty-select, [data-quiz-count]")) {
    const studyModule = getStudyModule();
    ...
    studyModule.quiz.counts = normalizeQuizCounts(Object.fromEntries(Array.from(document.querySelectorAll("[data-quiz-count]"))
      .map((input) => [input.dataset.quizCount, input.value])));
    studyModule.quiz.cursor = 0;
    studyModule.activeQuizTopic = studyModule.quiz.scope;
    saveWorkspace();
...
document.addEventListener("input", (event) => {
  if (!event.target.matches("#quiz-fill-answer, #quiz-short-answer")) return;
  const question = getCurrentQuizQuestion();
  if (!question) return;
  const studyModule = getStudyModule();
  studyModule.quiz.drafts = studyModule.quiz.drafts || {};
  studyModule.quiz.drafts[question.id] = event.target.value;
  saveWorkspace();
});
